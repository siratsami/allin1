...
echo "URL: $httpr" | tee curl/$1_curls; done
echo ""
...
for server in $(cat httprobe/$1_https); do curl $server -H "Host: $1" -I | grep -e "HTTP/" -e "Content-Length" | tee virtualh/$1_virtuals
